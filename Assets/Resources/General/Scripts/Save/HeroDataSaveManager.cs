...
public class HeroDataSaveLoadManager : MonoBehaviour
{
    ...
    // 保存单个英雄到文件
    public void SaveHeroData(HeroDataStruct heroDataStruct)
    {
        // 打开文件
        HeroDataList heroDataList = LoadHeroData();
        // 创建新的英雄信息文件
        HeroData heroData = new(heroDataStruct);
        // 查找是否已经存在该英雄
        int existIndex = heroDataList.heroes.FindIndex(h => h.heroID == heroDataStruct.heroID);
        // -1 说明不存在
        if (existIndex == -1) Debug.Log("该英雄不存在！无法保存数据,请先获得英雄");
        else heroDataList.heroes[existIndex] = heroData;
        // 存入文件
